The below if the README for the nvim config of Jeff Kreeftmeijer, but I'll be looking to start from scratch and build up my own config.
There's going to be some stuff that I definitely want to keep, like:
- 'kaplanz/retrail.nvim' # trim whitespace
- 'junegunn/fzf', { 'do': { -> fzf#install() } } # fzf is love, fzf is life
- 'junegunn/fzf.vim' # jup
- 'bluz71/vim-nightfly-colors', { 'as': 'nightfly' } # just a neat colorscheme.
